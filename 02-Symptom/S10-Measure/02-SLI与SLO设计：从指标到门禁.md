...
- **本篇系列角色**：S10-Measure 子系列的**工程落地下篇**（与 S10-01 概念篇配套）
- **强依赖**：[S10-01-症状机制](01-症状机制.md) 度量基础（先读完，否则概念会卡）
- **承接自**：[00-Meta/学习路线-稳定性架构师.md](../../00-Meta/学习路线-稳定性架构师.md) §7 Phase 4 治理体系
- **衔接去**：[S10-03 发布门禁 SOP](03-发布门禁SOP.md)（本篇输出 SLI，S10-03 落地为门禁）
- **不重复内容**：不重复 S10-01 §2 稳定性度量基础
- **本篇贡献**：把 SLI / SLO / Error Budget / 门禁 4 大概念**全部立住** + 给出 5 个核心 SLI 的**可落地定义**
...
| 轮次 | 类别 | 决策 | 理由 | 影响范围 |
|:-----|:-----|:-----|:-----|:---------|
| 1 | 结构 | 单篇 350+ 行（v4 默认 300） | SLI/SLO 涉及 5 个核心指标 + 4 阶段门禁，必须展开 | 全文 |
| 1 | 结构 | 图表 3 张（SLI 计算口径 / SLO 阶梯 / 门禁流程）| 度量类文档必须可视化 | §2 / §3 / §4 |
| 2 | 硬伤 | 5 个 SLI 全部给出"分子分母 + 采集点 + 计算公式" | 反例 #4 模糊量化 | §2 |
| 2 | 硬伤 | SLO 必须给"99.xx%"具体数字 | 反例 #11 数据堆砌 | §3 |
| 3 | 锐度 | 删"建议""可以"，改"必须 / 推荐 / 选读" | 反例 #5 模糊量化 | §4-§6 全部 |
| 3 | 锐度 | Error Budget 必须给"扣分 / 报警 / 暂停变更"3 个动作 | 反例 #8 不可执行 | §5 |
