...
| 9 | **S08** | **AOSP 17 + K 6.18 稳定性机制全景：6 ART 硬变化 + 8 K 硬变化 + 5 联动 + 5 风险 + 5 步升级** | **演进对比专题**（§8 破例）| S00-S07 | ~1200 |
| 10 | **S09** | **性能 vs 稳定性 5 大横向专题：Binder 死锁 / IO 调度 / GC 卡顿 / 渲染卡顿 / 锁竞争** | **横切专题**（§8 破例）| S00-S08 | ~1000 |
| 11 | **S10** | **稳定性度量学 + 发布门禁：MTBF / 崩溃率 / ANR率 / 严重性 / 回归率 / Stability Score + 6 大门禁 + 治理闭环** | **总览篇 / 治理度量**（§8 破例）| S00-S09 | ~1000 |
...
Phase 5（演进对比专题 · §8 破例 · 2026-07-18 新增）
  → S08 AOSP 17 + K 6.18 稳定性机制全景（**架构师升级决策必读**）

Phase 6（横切专题 · §8 破例 · 2026-07-18 新增）
  → S09 性能 vs 稳定性 5 大横向专题（**oncall 第一反应表**）

Phase 7（总览篇 / 治理度量 · §8 破例 · 2026-07-18 新增）
...
| 轮次 | 类别 | 决策 | 理由 | 影响范围 |
|------|------|------|------|---------|
| 1 | 系列结构 | **新增 S08 演进对比专题**（§8 破例）| 架构师升级决策时需要"AOSP 17 + K 6.18 稳定性全景"——单症状视角（S00-S07）已散落 ART/K 硬变化，需要"横切" | 全系列 1 篇新增 |
| 1 | 决策 | S08 用 `android17-6.18` 基线（AOSP 17 官方 GKI 内核）| 2026-07-18 复盘查证 AOSP 官方 2026-03 公告确认 | 全系默认基线 |
| 1 | S08 结构 | 单篇 1500+ 行（v4 默认 300 行）| §8 演进对比专题型破例 | 仅 S08 |
| 1 | S08 结构 | 图表 5-7 张（v4 默认 4-6）| §8 演进对比专题型破例 | 仅 S08 |
...
| 1 | 系列结构 | **新增 S09 横切专题**（§8 破例）| oncall 时需要"症状-根因"映射表——单症状视角（S00-S07）已散落 5 大根因，需要"根因维度横切" | 全系列 1 篇新增 |
| 1 | S09 结构 | 单篇 1000+ 行（v4 默认 300 行）| §8 横切专题型破例 | 仅 S09 |
| 1 | S09 结构 | 5 大专题各 1 张图 + 1 张总览图 | §8 横切专题型破例 | 仅 S09 |
...
| 1 | 系列结构 | **新增 S10 总览篇 / 治理度量**（§8 破例）| 架构师治理决策时需要"度量 + 门禁 + 闭环"——单症状视角（S00-S07）已讲机制，缺治理视角 | 全系列 1 篇新增 |
| 1 | S10 结构 | 单篇 1000+ 行（v4 默认 300 行）| §8 总览篇 / 治理度量型破例 | 仅 S10 |
