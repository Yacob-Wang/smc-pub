...
- **强依赖**：
  - 阅读前**先读** [S00-稳定性症状总览](../04-卷4-诊断方法论与稳定性症状/22-稳定性调查方法论/S00-症状总览.md)（7 大症状的边界立住后再看路线更顺）
  - 也可独立阅读（本篇会自包含所有引用）
...
- **不重复内容**：
  - **不重复** [Reference/术语表.md](术语表.md) §1.1 防混淆口诀
...
| 顺序 | 文章 | 为什么放第一位 |
|:-----|:-----|:---------------|
| 1 | [Process/01-进程总览](../03-卷3-核心机制/13-进程与生命周期/13.B-进程生命周期/01-进程总览：从点图标看app进程的诞生消亡与全栈抽象.md) | 12 个时间点 × 4 层 = 一张图读懂所有稳定性问题都跑在这条链上 |
| 2 | [Process/08-进程稳定性风险全景](../03-卷3-核心机制/13-进程与生命周期/13.B-进程生命周期/08-进程稳定性风险全景与跨层治理.md) | 4 层风险矩阵 + 24+ 监控指标 + 7 类治理动作（**上层稳定的总览**） |
| 3 | [Reference/术语表.md](术语表.md) §1.1（防混淆口诀） | 4 行口诀背下来：ANR 主动 / HANG 被动、SWT 杀 SystemServer / ANR 杀 App、NE 用户态 / KE 内核态、REBOOT 是结果 |
...
| 顺序 | 文章 | 核心抓手 |
|:-----|:-----|:---------|
| 1 | **[S00-稳定性症状总览](../04-卷4-诊断方法论与稳定性症状/22-稳定性调查方法论/S00-症状总览.md)**（✅ 已完成，2026-07-18） | 7 大症状严格定义 + 系统栈映射 + cascade 链路 + 速查表 |
...
| **ANR_Detection** | [Input_Dispatch_Timeout_ANR_Deep_Dive](../04-卷4-诊断方法论与稳定性症状/23-ANR%20深度/Input_Dispatch_Timeout_ANR_Deep_Dive.md) | Input ANR 检测链路源码深挖 |
| **ANR_Detection** | [Service_ANR_Deep_Dive](../04-卷4-诊断方法论与稳定性症状/23-ANR%20深度/Service_ANR_Deep_Dive.md) | Service ANR 阈值与机制 |
| **ANR_Detection** | [No_Focus_Window_ANR_Deep_Dive](../04-卷4-诊断方法论与稳定性症状/23-ANR%20深度/No_Focus_Window_ANR_Deep_Dive.md) | No Focus 异常路径 |
| **Input** | [Input/06-InputANR](../03-卷3-核心机制/18-输入系统/18.A-Framework%20输入链路/06-InputANR.md) + [07-Input稳定性风险全景](../03-卷3-核心机制/18-输入系统/18.A-Framework%20输入链路/07-Input稳定性风险全景.md) | Input ANR 全景 + 治理 |
| **App/Handler** | [06-Handler与ANR](../03-卷3-核心机制/14-线程与%20Handler%20消息机制/06-Handler与ANR.md) | App 侧主线程为什么会卡 |
...
| 优先级 | 文章 | 原因 |
|:------|:-----|:-----|
| **⭐ 必读** | [01-概述与体系位置](../04-卷4-诊断方法论与稳定性症状/27-系统无响应（SWT%20·%20Watchdog）/01-Watchdog概述与体系位置.md) | 多层 watchdog 全景 |
| **⭐ 必读** | [03-Java-Watchdog核心机制](../04-卷4-诊断方法论与稳定性症状/27-系统无响应（SWT%20·%20Watchdog）/03-Java-Watchdog核心机制.md) | HandlerChecker 状态机 |
| **⭐ 必读** | [05-超时判定与杀进程链路](../04-卷4-诊断方法论与稳定性症状/27-系统无响应（SWT%20·%20Watchdog）/05-Watchdog超时判定与杀进程链路.md) | 杀进程策略三层（线程 → SystemServer → 整机） |
| **⭐ 必读** | [06-实战案例与排查体系](../04-卷4-诊断方法论与稳定性症状/27-系统无响应（SWT%20·%20Watchdog）/06-Watchdog实战案例与排查体系.md) | 真实 logcat + 修复模式 |
| 选读 | [02-多层Watchdog架构](../04-卷4-诊断方法论与稳定性症状/27-系统无响应（SWT%20·%20Watchdog）/02-多层Watchdog架构.md) | 内核 watchdog 联动 |
| 选读 | [04-内核Watchdog与watchdogd](../04-卷4-诊断方法论与稳定性症状/27-系统无响应（SWT%20·%20Watchdog）/04-内核Watchdog与watchdogd.md) | 软/硬死锁检测 |
| **加餐（必读）** | [BinderStarve.md](../04-卷4-诊断方法论与稳定性症状/27-系统无响应（SWT%20·%20Watchdog）/BinderStarve.md)（177KB） | Binder 饿死导致 SWT 的全栈分析——**稳定性架构师王牌案例** |

#### ⚙️ Handler / MessageQueue / Looper（8 篇，~6 小时）

| 优先级 | 文章 | 原因 |
|:------|:-----|:-----|
| **⭐ 必读** | [01-Handler消息机制总览](../03-卷3-核心机制/14-线程与%20Handler%20消息机制/01-Handler消息机制总览.md) | 主线程模型的"心脏" |
| **⭐ 必读** | [02-Looper与线程模型](../03-卷3-核心机制/14-线程与%20Handler%20消息机制/02-Looper与线程模型.md) | 线程消息循环 |
| **⭐ 必读** | [05-同步屏障与异步消息](../03-卷3-核心机制/14-线程与%20Handler%20消息机制/05-同步屏障与异步消息.md) | 同步屏障卡死的经典坑 |
| **⭐ 必读** | [06-Handler与ANR](../03-卷3-核心机制/14-线程与%20Handler%20消息机制/06-Handler与ANR.md) | ANR 的主线程根因 |
| **⭐ 必读** | [07-Handler稳定性风险全景](../03-卷3-核心机制/14-线程与%20Handler%20消息机制/07-Handler稳定性风险全景.md) | 11 类风险全梳理 |
| 选读 | [08-诊断工具与监控体系](../03-卷3-核心机制/14-线程与%20Handler%20消息机制/08-消息机制诊断工具与监控体系.md) | LooperPrinter 监控方案 |
| 加餐 | [HandlerThread泄露分析与防治](../03-卷3-核心机制/14-线程与%20Handler%20消息机制/HandlerThread泄露分析与防治.md) | 实战案例 |
| 加餐 | [LooperPrinter是否需要监控？](../03-卷3-核心机制/14-线程与%20Handler%20消息机制/LooperPrinter是否需要监控？.md) | 决策依据 |

#### 🎯 Input（8 篇，~8 小时）

| 优先级 | 文章 | 原因 |
|:------|:-----|:-----|
| **⭐ 必读** | [01-Input系统总览](../03-卷3-核心机制/18-输入系统/18.A-Framework%20输入链路/01-Input系统总览.md) | 触摸从驱动到 View 的完整链路 |
| **⭐ 必读** | [03-InputDispatcher](../03-卷3-核心机制/18-输入系统/18.A-Framework%20输入链路/03-InputDispatcher.md) | **5s Input ANR 的核心调度器** |
| **⭐ 必读** | [06-InputANR](../03-卷3-核心机制/18-输入系统/18.A-Framework%20输入链路/06-InputANR.md) | 5s ANR 触发条件 + 判定逻辑 |
| **⭐ 必读** | [07-Input稳定性风险全景](../03-卷3-核心机制/18-输入系统/18.A-Framework%20输入链路/07-Input稳定性风险全景.md) | 11 类风险 |
| 选读 | [02-EventHub与InputReader](../03-卷3-核心机制/18-输入系统/18.A-Framework%20输入链路/02-EventHub与InputReader.md) | 内核 → Framework 边界 |
| 选读 | [04-InputChannel与跨进程投递](../03-卷3-核心机制/18-输入系统/18.A-Framework%20输入链路/04-InputChannel与跨进程投递.md) | 跨进程链路 |
| 选读 | [05-View事件分发](../03-卷3-核心机制/18-输入系统/18.A-Framework%20输入链路/05-View事件分发.md) | 客户端分发 |
...
| 优先级 | 文章 | 原因 |
|:------|:-----|:-----|
| **⭐ 必读** | [01-NativeCrash总览](../../04-卷4-诊断方法论与稳定性症状/25-Native 异常/01-NativeCrash总览.md) | debuggerd 入口 |
| **⭐ 必读** | [02-Linux信号机制](../../04-卷4-诊断方法论与稳定性症状/25-Native 异常/02-Linux信号机制.md) | 6 种致命信号根因 |
| **⭐ 必读** | [04-debuggerd与Tombstone](../../04-卷4-诊断方法论与稳定性症状/25-Native 异常/04-debuggerd与Tombstone.md) | tombstone 抓取链路 |
| **⭐ 必读** | [06-Tombstone深度解读](../../04-卷4-诊断方法论与稳定性症状/25-Native 异常/06-Tombstone深度解读.md) | 16 段结构怎么读 |
| **⭐ 必读** | [08-APM集成与治理](../../04-卷4-诊断方法论与稳定性症状/25-Native 异常/08-APM集成与治理.md) | 监控落地 |
| 选读 | [03-内存管理与保护](../../04-卷4-诊断方法论与稳定性症状/25-Native 异常/03-内存管理与保护.md) | 内存越界原理 |
| 选读 | [05-栈回溯与符号化](../../04-卷4-诊断方法论与稳定性症状/25-Native 异常/05-栈回溯与符号化.md) | unwinding |
| 选读 | [07-检测工具体系](../../04-卷4-诊断方法论与稳定性症状/25-Native 异常/07-检测工具体系.md) | asan / fuzzer / 内存检测 |
...
| 场景 | 必读 |
|:-----|:-----|
| **冷启动慢** | [01](../03-卷3-核心机制/13-进程与生命周期/13.B-进程生命周期/01-进程总览：从点图标看app进程的诞生消亡与全栈抽象.md) + [02](../03-卷3-核心机制/13-进程与生命周期/13.B-进程生命周期/02-AMS-冷启动判定与进程启动链路.md) + [03](../03-卷3-核心机制/13-进程与生命周期/13.B-进程生命周期/03-Zygote-Android进程工厂.md) + [04](../03-卷3-核心机制/13-进程与生命周期/13.B-进程生命周期/04-应用进程首生-fork到ActivityThread.md) |
| **OOM / lmkd** | [06-Framework视角的Kernel进程接口](../03-卷3-核心机制/13-进程与生命周期/13.B-进程生命周期/06-Framework视角的Kernel进程接口_procfs_cgroup_pidfd.md) §9.1 + [07-调度与资源](../03-卷3-核心机制/13-进程与生命周期/13.B-进程生命周期/07-调度与资源：CFS与进程生死.md) |
| **进程死 / crash 重启** | [08-进程稳定性风险全景](../03-卷3-核心机制/13-进程与生命周期/13.B-进程生命周期/08-进程稳定性风险全景与跨层治理.md) |
...
| **Kernel panic / oops** | [Linux_Kernel/Process/13-进程调试与稳定性关联](../03-卷3-核心机制/13-进程与生命周期/13.B-进程生命周期/13-进程调试与稳定性关联.md) + [Linux_Kernel/Process/11-信号机制](../03-卷3-核心机制/13-进程与生命周期/13.B-进程生命周期/11-信号机制_从产生到投递.md) |
...
> - **症状入口**：[S00-稳定性症状总览](../04-卷4-诊断方法论与稳定性症状/22-稳定性调查方法论/S00-症状总览.md) · [README-Stability系列.md](../README.md)
