...
- **强依赖**：[00-总览 01-ART 总览 v2 升级版](01-ART总览：稳定性架构师的全局视角.md) §1.1（ART Runtime 核心机制）
...
- **不重复内容**：
  ...
  - 不深入 GC / 类加载（→ 03 子模块）
- **本篇是 v2 升级版**：**原 v1 旧文（639 lines）已按 本规范 + AOSP 17 + 6.18 重写**
...
| 第 1 轮 · 结构 | **v2 升级策略** | 保留 v1 精华（Dex 文件结构 + Dalvik 指令集） + 增补 ART 17 解释器优化引用 + 替换基线 | §7.3 批次升级 | 仅本篇 |
...
第 00-总览 [v2 升级版](01-ART总览：稳定性架构师的全局视角.md) §1.1 已讲 ART Runtime 整体机制。
...
- **v1 旧版**（639 lines）：AOSP 14 + 5.10/5.15 基线，无 本规范必含项
