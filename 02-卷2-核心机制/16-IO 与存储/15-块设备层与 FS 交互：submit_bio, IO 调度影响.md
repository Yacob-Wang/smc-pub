...
- 不重复内容:本篇**不重复 ext4 / f2fs / erofs 内部机制**(见 [12-14](12-ext4%20文件系统架构：磁盘布局,%20extent,%20journaling.md))、**不重复 IO 调度器算法**(见 [IO 02 调度器](02-IO调度器与多队列架构.md))、**不重复设备性能**(见 [IO 09 设备性能](09-存储设备与IO性能：UFS、eMMC、NVMe命令队列与延迟特性.md))
...
|---------|---------|---------|----------------|
| 块设备 IO 慢 | UFS 队列满 | 应用卡顿 | [IO 09 设备性能](09-存储设备与IO性能：UFS、eMMC、NVMe命令队列与延迟特性.md) |
