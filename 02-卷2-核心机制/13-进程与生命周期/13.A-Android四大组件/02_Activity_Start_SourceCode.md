...
> 跨系列引用：zygote fork 出 ActivityThread 的完整首生链路见 [Process 04-应用进程首生]（待定，Process 系列未发布）；`am start` 调用的就是 A02 链路，命令行全景见 [AmCommand 01-am 命令全景]（待定，AmCommand 系列未发布）；冷启动时 ContentProvider 早于 Application 初始化的时序见 [ContentProvider 初始化](C02_ContentProvider_Init.md) §1（C02）。
