...
**衔接去**：[10-ART17分代GC强化专章 v2](../10-ART17分代GC强化专章-v2.md) §3 专章 ART 17 分代 GC 强化；[09-GC诊断与治理](../09-GC诊断与治理/01-ART日志与GC诊断.md) 深入 GC 监控工具链。
...
```java
// 优化 1：减少 dirty 对象（复用 StringBuilder）
...
// 优化 2：控制堆大小（256MB 是默认推荐）
// adb shell setprop dalvik.vm.heapgrowthlimit 256m

// 优化 3：减少线程数（用线程池）
ExecutorService executor = Executors.newFixedThreadPool(8);

// 优化 4：避免 finalize()（用 PhantomReference + Cleaner 替代）
```
...
**典型用户反馈**：App 在滑动列表时偶发性卡顿，每次 50-100ms。

**典型根因**：CMS Remark 阶段 STW（dirty 对象过多）。

**典型修复**：优化对象创建（复用对象）+ 升级到 Android 17+ GenCC（STW < 2ms）。
...
│  ───────────────────────────────────────────────               │
│  AOSP 14 CMS：Initial Mark 串行扫描所有 GC Root（~30K 对象）    │
│  AOSP 17 GenCC：并发扫描 + Class Unloading 并发化              │
│                + Read Barrier 替代部分 Write Barrier           │
│                                                                │
│  优化 2：Remark 50ms → 20-30ms（Full GC 时；提升 40-60%）       │
│  ───────────────────────────────────────────────               │
│  AOSP 14 CMS：Remark 必须串行处理所有 dirty 对象 + 重新扫描     │
│  AOSP 17 GenCC：Young GC 不走 Remark（无 dirty 对象）          │
│                Old → Young 引用由 Remembered Set 记录，O(1) 查  │
│                Full GC 走 Remark 时，dirty 对象数 1M → 200-500K │
│                                                                │
...
class ConcurrentCopying : public GarbageCollector {
  void InitializePhase() {
    concurrent_class_unloading_ = true;  // 并发卸载无用类
  }
  void MarkingPhase() {
    // Read Barrier 替代部分 Write Barrier
...
**架构师视角**：ART 17 STW 优化是 **Minor GC 不走 Remark** 的结果。大部分 App 99%+ 的 GC 是 Minor GC，**用户实际感受：滑动卡顿从 50-100ms 降至 < 2ms**。
...
**对 STW 的间接影响**：Native 堆占用降低 → ART 进程总内存降低 → LMK 压力降低 + 分配延迟降低 → GC 触发期间业务线程暂停概率降低。
