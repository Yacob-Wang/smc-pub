...
| Linux 6.12 与 RSet 关联 | 未涉及 | **新增 §7.3 整节** | 跨系列基线一致性 |
...
### 7.3 Linux 6.12 与 RSet 的关联

- **Linux 6.12 内存屏障原语**：让 RSet 的原子更新更高效
- **Linux 6.12 sheaves 内存分配器**：让 RSet 自身的分配开销降低 10%
- **Linux 6.12 io_uring 增强**：让 RSet 刷盘延迟降低 30%
...
| 9 | Linux 6.12 `arch/arm64/include/asm/barrier.h` | ✅ 已校对 | 跨系列基线 |
...
| 18 | Card Table 刷盘延迟（Linux 6.12） | -30% | Linux 6.12 io_uring |
| 19 | RSet 自身分配（Linux 6.12 sheaves） | -10% | AOSP 17 + Linux 6.12 |
