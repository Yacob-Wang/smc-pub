...
- **强依赖**：
  - 必先读 [S00-稳定性症状总览](../22-稳定性调查方法论/S00-症状总览.md) §2.2 七大症状横向对比表（**HANG 在"沉默的杀手"位置**）
  - 必先读 [S01-ANR](../S01-ANR/01-症状机制.md) §2.1（ANR vs HANG vs SWT 决策树）
- **承接自**：[S01-ANR](../S01-ANR/01-症状机制.md) 已覆盖"主动检测"类（ANR / SWT）；本篇覆盖**"无主动检测"类**（HANG）
- **衔接去**：
...
- **不重复内容**：
  ...
  - **不重复** [Linux_Kernel/Process](../01-Mechanism/Kernel/Process/) 对 Kernel 死锁机制深挖
  - **不重复** [Linux_Kernel/Binder](../01-Mechanism/Kernel/Binder/) 对 binder 死锁深挖
  - **不重复** [Linux_Kernel/IO](../01-Mechanism/Kernel/IO/) 对 IO 调度深挖
...
- **上一篇**：[S01-ANR](../S01-ANR/01-症状机制.md) 已覆盖"主动检测"类
...
**剩余 3 篇**：
- [S04-SWT](../S04-SWT/01-症状机制.md)：SystemServer 卡死（Watchdog 触发的症状链）
- [S06-REBOOT](../S06-REBOOT/01-症状机制.md)：重启（KE 的结果态 + cascade 链路）

**写作顺序**：S00 → S01 → S02 → S03 → S07 → S05 → **S04 / S06**

---

> **系列导航**：[← S07-KE](../S07-KE/01-症状机制.md) | [本系列 README](../README.md) | [S04-SWT →](../S04-SWT/01-症状机制.md)
>
