...
| # | 场景 | 占比 | 用户报 | 详见 |
|:-:|------|:----:|--------|------|
| 1 | **P0-1:Java OOM 闪退** | 35% | "App 打开 5 次后闪退" | [26.2](file:///E:/smc-pub/04-卷4-稳定性症状/38-内存与 OOM/02-Java-OOM-堆溢出-大对象-Bitmap-线程数超限.md) |
| 2 | **P0-2:进程被杀** | 30% | "App 莫名被杀,重启" | [26.4](file:///E:/smc-pub/04-卷4-稳定性症状/38-内存与 OOM/04-进程被杀-LMK判定链路与优先级误配型误杀.md) |
| 3 | **P0-3:系统卡顿** | 35% | "App 莫名卡顿 / 系统整个慢" | [26.5](file:///E:/smc-pub/04-卷4-稳定性症状/38-内存与 OOM/05-内存压力连锁反应-GC抖动-掉帧-ANR.md) |

(表 1-1:Oncall 内存专项 3 大常见场景)

**关键事实**:**内存 P0 现场不能"再来一次"**——30 分钟内一次抓全,过后水位变化证据失效(详见 [26.6 §1](file:///E:/smc-pub/04-卷4-稳定性症状/38-内存与 OOM/06-内存现场采集与水位治理.md))。
...
**上一篇**:[26.11 Native 调试基础-GWP-ASan-HWASan-MTE 调试验证](file:///E:/smc-pub/04-卷4-稳定性症状/38-内存与 OOM/11-Native-调试基础-GWP-ASan-HWASan-MTE-调试验证.md)
**下一篇**:[26.13 APM SDK 内存采集与自动化监控脚本](file:///E:/smc-pub/04-卷4-稳定性症状/38-内存与 OOM/13-APM-SDK-内存采集与自动化监控脚本.md)——补全系列收口子篇
