...
| 本系列涉及主题 | 跨系列引用 | 引用理由 |
|--------------|------------|---------|
| Class Linker / dex 加载 | [`../01-Mechanism/Runtime/`](../01-Mechanism/Runtime/)(如存在) | ART 编译 / 链接细节 |
| IApplicationThread Binder | [`../../Android_Framework/Binder/`](../Binder/) | ApplicationThread 后续消息分发(本篇 §3.1 T6.5) |
| Window 销毁/重建 | [`../01-Mechanism/Framework/Window/`](../Window/) | 进程驻留期间 Window 生命周期 |
| 冷启动期间 thread dump | [`../01-Mechanism/Framework/Input/`](../Input/) | ANR 后 SIGQUIT 触发 thread dump 链路 |
| dex2oat / Dalvik Cache | [`../01-Mechanism/Kernel/Partition/`](../Partition/) | `/data` 分区布局影响 dalvik-cache 持久性 |
| 调度延迟 / ANR | [`../04-Tool/Watchdog/`](../Watchdog/)、[`../ANR_Detection/`](../ANR_Detection/) | 进程级 ANR 检测 |
