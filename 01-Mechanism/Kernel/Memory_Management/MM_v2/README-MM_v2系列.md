...
> 核心问题:本篇章是 v2 主序列(01-13)落地后,**根据 6 大缺口**补强出来的新文章 + 已有文章补强子节。两条主线:
>
> - **14 篇(类型学)**:同一个 VMA 体系(02 篇)按"进程类型"反过来拆开看 ——Android 6 大类进程(zygote / system_server / app / native 守护进程 / kernel 线程 / init)的 /proc/pid/maps 分别长什么样?
> - **15 篇(治理)**:FWK 层如何标识记录内存 + **不杀进程情况下**,如何在线上识别/隔离/梳理内存问题(回答"线上动态治理"这个 v2 主线唯一未覆盖的口子)
>
> 此外,14 篇内部还补了 §15/§16 两个子节(zygote / system_server 单独画像),07 篇内部补了 §5.6 cgroup v2 实际配置子节。
...
| **§15. 补强:Zygote 内存膨胀的专门治理(线上)** | zygote 5 大根因 / preload 裁剪 / 远程动态释放 | `frameworks/base/config/preloaded-classes` | zygote 治理的 4 大误区 |
| **§16. 补强:system_server 单独画像与治理(线上)** | 5 大子模块 / cgroup 冻结 / oom_score_adj 锁定 | `frameworks/base/services/core/java/com/android/server/am/ActivityManagerService.java` | system_server 治理的 4 大误区 |

### [15-线上动态内存治理:不杀进程下的诊断与梳理](15-线上动态内存治理：不杀进程下的诊断与梳理.md)

> **v2 唯一明确缺口的补位**:FWK 层如何"看见"内存 + 线上不杀进程如何"治理"内存。

| 章节 | 内容 | 核心源码路径 | 稳定性关联 |
| --- | --- | --- | --- |
| **0. 写在最前:为什么需要"不杀进程"治理** | 杀进程是最粗暴的兜底;线上应能"识别 + 隔离 + 梳理"三步走 | — | 治理哲学:不止 LMKD |
| **1. FWK 层如何标识记录内存:ProcessRecord 内存视图** | ActivityManager 内存账本(14 字段)、MemoryFactor 分级、lastCachedPss / memAvailable | `frameworks/base/services/core/java/com/android/server/am/ProcessRecord.java`、`ProcessList.java` | FWK 层数据采集点 |
| **2. 线上诊断:不杀进程下识别内存问题** | 4 步法:baseline 对比 → TopN 锁定 → 时序归因 → cgroup 节点级 pressure | `frameworks/base/services/core/java/com/android/server/am/ActivityManagerService.java` | 排查路径 |
| **3. 线上隔离:不杀进程下"框定"问题范围** | cgroup v2 节点级 memory.max/high/min 软硬隔离 | `kernel/cgroup/memcontrol.c` | 治理手段 |
| **4. 线上动态梳理:不杀进程下"治理"内存** | 5 大手段:显式 GC / dump-heap / ART trim / preload 远程释放 / watermark 调整 | `art/runtime/gc/heap.cc`、`libart.so` | 治理手段 |
| **5. 实战 3 例(基于真实脱敏)** | system_server 2GB 内存冻结 / zygote 1.5GB 印钞机瘦身 / 相机泄漏 cgroup 隔离 | 多个 | 端到端示范 |
| **6. 监控与告警(配套)** | 3 层监控:ProcessRecord 维度 / cgroup 节点维度 / 全机 PSI 维度 | — | 持续治理 |
| **7. 总结:架构师视角 5 条 Takeaway** | FWK 是"看"层,LMKD 是"杀"层,治理在中间 | — | 收口 |
| **附录 A** | ProcessRecord 内存字段速查 | — | 工具书 |
| **附录 B** | cgroup v2 节点级操作命令集 | — | 工具书 |
| **附录 C** | 实测 perfetto heapprofd 命令集 | — | 工具书 |
| **附录 D** | 案例脱敏素材模板 | — | 工具书 |

> **跨篇衔接**:
> - 往前 → [14 §15/§16 zygote/system_server 单独画像](#14-android-进程内存类型学)(本篇 15 的"治理"对象)
> - 横向 → [07 §5.6 cgroup v2 实际配置](07-PSI、vmpressure、memcg 压力传递.md)(本篇 15 的 cgroup 操作基础)
> - 往后 → [Process 09-杀进程慢的根因定位实战](../Process/09-杀进程慢的根因定位实战.md)(杀进程视角的反向参照)

### [07-PSI / vmpressure / memcg 压力传递](07-PSI、vmpressure、memcg 压力传递.md) — 已补强

| 新增子节 | 内容 | 核心源码路径 | 稳定性关联 |
| --- | --- | --- | --- |
| **§5.6.1 cgroup v1 vs v2 Android 现状** | Android 14+ 全面切 cgroup v2,接口变化 | `kernel/cgroup/cgroup.c` | 接口迁移 |
| **§5.6.2 实际树形结构(TECNO KM9 / Android 14 实测)** | `init.scope/` `system.slice/` `apps.slice/` `top-app.slice/` 四层 | — | 树形结构 |
| **§5.6.3 cgroup v2 节点级 memory 接口(完整)** | 19 个 memory.* 文件速查表 | `kernel/cgroup/memcontrol.c` | 操作入口 |
| **§5.6.4 关键治理命令集(完整)** | 9 步操作:找 cgroup 节点 → 读 PSI → 设 max/high/min | — | 实操工具 |
| **§5.6.5 cgroup v2 治理 5 大常见坑** | max=0 立即 OOM / min 太大饿死其他 / 单位错 / 嵌套不识别 / 清理不全 | — | 避坑 |
| **§5.6.6 与 06/12/15 的衔接** | 06 LMKD / 12 风险 / 15 治理的 cgroup 协作 | — | 跨篇衔接 |
| **§5.6.7 与 14 进程类型学的衔接** | zygote / system_server / app / native 在 cgroup v2 树中的位置 | — | 跨篇衔接 |
...
6. **[14 进程类型学]** — 按进程类型看 maps，跟 02 互为正反面，看完能一眼判断是 zygote / system_server / app / native。
7. **[15 线上动态治理]** — 唯一直接回答"不杀进程怎么治理"的文章，6 大缺口中最大那块的补位。

### 如果你要系统学习，按顺序阅读 01 → 15：
...
13 诊断工具（垂直工具箱）
    ↓
14 进程类型学（按进程类型展开 02 的 VMA 体系）
    ↓
15 线上动态治理（不杀进程下，如何识别/隔离/梳理）
```
...
- [x] Step 6: v2 补强（根据 6 大缺口审计）— 2026-07-20
  - 14 篇新增 §15 zygote 治理 + §16 system_server 单独画像
  - 07 篇新增 §5.6 cgroup v2 实际配置（5.6.1-7 全套）
  - **新增 15 篇-线上动态内存治理:不杀进程下的诊断与梳理**（补"FWK 层标识 + 线上动态治理"这块唯一未覆盖的口子）

**全 15 篇(13 主序列 + 2 补篇) + 14 篇/07 篇补强子节 100% 落地，目录位置**：`Linux_Kernel/Memory_Management/MM_v2/`。**总规模** ≈ 1.2 MB / ~17 万字。
...
- 15 篇 v2 候选(线上场景下)进一步扩写:多 app 联动 / 前台后台优先级 / 杀进程过渡到软治理的方法论
- Process 09 ↔ MM_v2 15 双向衔接(杀进程视角 vs 治理视角的"上下游")
