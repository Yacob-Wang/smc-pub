...
- 篇尾"自检报告"用 `<!-- AUTHOR_ONLY:SELFCHECK -->` 包裹（不计入正文）
...
**架构师视角**：
- **同一类问题可能跨多个子系统**——如"GC 频繁"通常由 Old Gen 满导致，但 AOSP 17 软阈值让 Young Gen 频繁 GC 也会让"GC 频繁"误诊
...
### 6. 公开站剥离模拟（py -c "import re; ..."）

```python
import re
with open('03-ART堆与GC的设计动机：为什么这样设计.md', 'r', encoding='utf-8') as f:
    content = f.read()

# 1. 剥 5 段作者前言
pat_author = re.compile(r'<!-- AUTHOR_ONLY:START -->.*?<!-- AUTHOR_ONLY:END -->\n?', re.DOTALL)
cleaned = pat_author.sub('', content)

# 2. 剥自检报告
pat_selfcheck = re.compile(r'<!-- AUTHOR_ONLY:SELFCHECK -->.*', re.DOTALL)
cleaned = pat_selfcheck.sub('', cleaned)

# 3. 验证顶部 blockquote 完整保留
assert cleaned.startswith('# ART 堆与 GC'), 'Title should be preserved'
assert '> 系列第 03 篇' in cleaned[:200], 'Top blockquote should be preserved'

# 4. 验证 5 段作者前言完全剥离
assert '本篇定位' not in cleaned[:5000] or '不重复内容' in cleaned[:5000]  # 后者是底部显式
# 实际上 AUTHOR_ONLY 段里的 "本篇定位" 应被剥掉
assert '校准决策日志' not in cleaned[:5000] or cleaned.find('校准决策日志') > 10000, '决策日志应被剥'
assert '角色设定' not in cleaned[:3000] or cleaned.find('角色设定') > 10000, '角色设定应被剥'
assert 'AUTHOR_ONLY:START' not in cleaned, 'No AUTHOR_ONLY:START marker in public view'
assert 'AUTHOR_ONLY:END' not in cleaned, 'No AUTHOR_ONLY:END marker in public view'
assert 'SELFCHECK' not in cleaned, 'No SELFCHECK marker in public view'

print('剥除前:', len(content), 'chars')
print('剥除后:', len(cleaned), 'chars')
print('剥除:', len(content) - len(cleaned), 'chars')
print('✅ 公开站剥离验证通过')
```
...
```bash
# 字数
wc -m "03-ART堆与GC的设计动机：为什么这样设计.md"  # 应在 50000-60000 字符
# 行数
wc -l "03-ART堆与GC的设计动机：为什么这样设计.md"  # 应 ≥ 300 行
```

### 8. 破例决策自检

- 3 个实战案例（规则 1-2 个）— 已记录破例理由
- memorylimiter.cpp 路径 🟡 — 已记录 09 篇校准
- 不会传染（仅本篇）
