...
>
> **源码基线**:AOSP `android-17.0.0_r1`(代号 CinnamonBun)+ Kernel `android17-6.18` GKI(主线)+ `android17-6.19`(backport);旧基线 `android14-5.10/5.15` / `android15-6.1/6.6` 作历史对照(本篇涉及 `fs/read_write.c`、`mm/filemap.c`、`block/blk-mq.c`、`mm/page_io.c`;各内核版本差异见 §4.4 PageCache 与 xarray 迁移、§7 blk-mq 多队列变化)
>
...
| 轮次 | 类别 | 决策 | 理由 | 影响范围 |
|------|------|------|------|----------|
| 1 | 结构 | v6 → v5 回退:5 段前言恢复(本篇定位/角色设定/上下文/写作标准/校准日志) | 用户决策:偏好 v5 风格(cgroup 6 篇范本) | 顶部 + AUTHOR_ONLY 段 |
| 2 | 硬伤 | 顶部 blockquote 11 行:系列/基线/读者/承接/衔接/强依赖/规范 7 类元信息 | v5 §3 模板要求"强制开头段"+ 用户偏好 cgroup 风格 | 顶部 blockquote |
| 2 | 硬伤 | 跨篇引用 `MM_v2` / `FS/` 旧命名 → `Memory` / `FileSystem` 新命名 | 项目目录 2026-07 已统一 | 跨篇链接 |
| 3 | 锐度 | "通常" 5 处全清 → 改"经验值/具体看/多见" | v5 §3 硬性要求 #5 + §5 反例 #5 硬伤 | 全文 5 处 |
| 3 | 锐度 | "往往" 2 处全清 → 改"主要/常" | v5 §3 硬性要求 #5 | 全文 2 处 |
| 3 | 锐度 | "接下来" 1 处全清 → 改"然后/之后" | v5 §3 硬性要求 #5 | 全文 1 处 |

## 角色设定

我是一名 **Android 稳定性架构师**,正在系统学习 IO 子系统。本篇是 IO 系列第 1 篇,主题是"从进程 read/write 到磁盘的完整链路"——为后续 10 篇(调度器、Block、cgroup、MM 耦合、进程耦合、程序加载、Android 栈、设备、风险、eBPF)建立统一的心智地图。
...
### 硬性要求(8 条)
1. **目标读者**:资深架构师。不需要解释"什么是进程""什么是内核",但需要解释 IO 特有的术语(bio/request/queue/Page Cache/IO 调度)
2. **每个章节先讲"是什么、为什么需要它、解决什么问题"**,然后再深入源码
3. **源码标注**:每个技术论点标注 `fs/read_write.c` / `mm/filemap.c` / `block/blk-mq.c` 等具体路径 + 内核版本基线
4. **关联实战**:每个知识点必须关联到真实工程问题(Crash/ANR/OOM/性能退化/IO hang)
5. **量化描述**:必须给具体数字 + 来源(不写"通常""大约"等模糊用词)
6. **版本基线**:AOSP 17 + android17-6.18(对照 5.10-6.6 历史)
7. **工程基线**:涉及可调参数时,给出"工程默认值"+"选用准则"
...
### 图表密度
- 1 张核心架构图(IO 全链路)
- 2-3 张时序图(open/read/write)
- 1 张 IO↔MM↔Process 耦合图
- 1 张风险地图 + 2 张对比表
...
线上故障归因里，**IO 几乎从不"直接报错"**,它多以以下 5 种"伪装"出现:

| 表面现象 | 真实根因（IO 占主） | 本系列对应文章 |
|---------|--------------------|--------------|
...
> **注意**：这是"概念分层"，实际内核中第 2-4 层的函数调用栈深度多在 10 层以内,第 5-6  层的关键耗时（设备层）才是 IO 延迟的主因。
...
- 然后如果 `vfs_read` 阻塞，整个进程进入 D 状态（[06-IO 与进程](06-IO与进程的深度耦合：D状态、iowait、IO-hang、进程阻塞.md) 详解）。
...
> **踩坑**：Java 的 `Future.get()`、OkHttp 的 enqueue 等"异步 API"，底层多采用同步 IO + 工作线程池——并不是真正的内核异步 IO。
...
| **blk-mq nr_hw_queues** | 设备硬件队列数 | UFS 常见 2-4 | 与设备能力匹配 |
