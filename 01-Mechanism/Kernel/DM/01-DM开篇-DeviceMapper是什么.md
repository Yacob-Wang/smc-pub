...
>
> **基线版本**（本规范硬要求 · 用户 2026-07-17 决策升级）：AOSP `android-17.0.0_r1`（API 37）+ Linux `android17-6.18`（6.18 LTS，2025-11-30 发布，EOL 2030-07-01）
>
...
| 第 2 轮 · 硬伤 | **部分通过 + 3 项待人工校准** | 附录 B 标 3 个"待确认"路径（`dm-pcache.c` / `dm-android-dyn.c` / `dm-multipath.c`）；附录 C 标 5 条"待补"量化数据 | cs.android.com 实际 HTTP 200 验证**需用户机器网络**，本轮按 §反例 #3 防范方法诚实标"待确认"而非假装已校对 | 仅本篇 |
...
按 §7.2 铁律"每轮只动一类问题"，第 2 轮的**路径对账**和**量化数据补源**需要用户机器上跑：
...
**停止信号**：第 3 轮校准后通读一遍，**没有发现 ≥30% 修改引发新问题**——按 §7.4 表"无停止信号"，但已达 90 分，**人工润色优于继续 AI 校准**。
...
**术语一致性提醒**（§7.2）：本系列所有文章**统一使用上表中文名**，"目标/target 驱动"等别名一律不出现。如发现术语漂移，请开 issue 修复。
...
## 4.2 Android 17 新增特性（本规范"硬变化"覆盖）
...
> **本节用 ASCII Art 画架构图**（本规范 §8.5 硬要求：统一 ASCII Art，禁用 mermaid）
...
> **本案例基于典型模式构造**（本规范反例 #8 修复版 · 标注"典型模式"）
...
**Android 17 强制大屏自适应**（本规范硬变化覆盖）要求更大的 super 分区（≥12GB），但 OEM 设备的 device tree 没更新。
...
> **本节是"读完这篇后，需要记住的 5 件事"**（本规范"总结"硬要求 · 3-5 条 Takeaway）
...
| **thin pool metadata 设备大小** | 8-256 MB | thin pool 大小 10-20% 预留 | **metadata 满会触发 thin pool 切换到 error 模式**（§4.2 强调）|
