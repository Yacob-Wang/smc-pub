...
| 第 1 轮 · 结构 | **破例记录**（§8）| 实战案例 2 个（规则 1-2 个上限）；图表 5 张 | **破例理由**：横切专题型，Android 场景丰富，1 案例覆盖不足 | 仅本篇 | 否 |
...
**Android 17 新增 3 个 DM 应用**（本规范硬变化覆盖）：
...
- **大屏设备 super 分区尺寸规划**是动态分区的核心——§3.4 详解
...
| 映射表条目 | 4-6 段 | **8-10 段**（§5 实战案例）|
| fs_mgr 加载时延 | 2-3 秒 | **3-8 秒** |
| timeout 风险 | 低 | **中**（5 秒默认可能不够）|

**对读者有什么用**：

- **大屏设备启动慢** = 第一嫌疑是**fs_mgr 加载 super 映射表**（§4 已讲）
- **大屏 super 映射表设计**：**合并相邻 linear 段**（§8 调优篇会深入）
...
- **dm-verity 在 init 阶段加载**（§4 启动篇详解）
- **verity 校验是 read 时实时校验**——**不是启动时一次性校验**
- **校验失败 = 整个 bio 失败**（默认行为）

## 4.3 dm-verity 的稳定性风险

**dm-verity 失败 5 大类**（§4 启动篇实战案例 1 已讲部分）：

| 失败模式 | 占比 | 排查方法 |
|---------|------|---------|
| **root hash mismatch** | 60% | OTA 包构建问题（§4 已讲）|
...
- **"dm-verity verification failed"** = 几乎都是 **OTA 包构建问题**——§4 启动篇实战案例
...
- **`logcat -s vold:V` 是关键排查命令**（§9 排障篇会深入）
...
**Virtual A/B 失败 5 大类**（§6 已讲 snapshot exception store 满）：
...
| **映射表碎片化** | 8-10 段 linear 拆分，bio 拆分风险（§5 实战案例）|
...
**Android 17 keystore 服务启动失败**（§4 启动篇提到 PQC 加密新机制）——**vold 拿不到 CE/DE 密钥 → dm-crypt 无法加载**。
