...
| 轮次 | 类别 | 决策 | 理由 | 影响范围 |
|------|------|------|------|----------|
| 1 | 结构 | 破例为"演进型"：3 张图 + 2 张对比表（v5 §9 演进型破例） | 主题是历史演进，机制讲解与版本对比表更高效 | 仅本篇 |
...
## 章节结构（v5 §3 标准 8 章 + v5 §9 演进型破例）
- **破例**：图表密度降到 3 张核心图 + 2 张对比表（v5 §9 演进型）
- 章节：背景与定义 → 演进前史 → cgroup 诞生 → v1 时代 → v2 时代 → Android 引入 → v1 vs v2 关键差异 → 演进对稳定性的启示
- 实战案例 1 个（v5 §9 演进型允许）：v1 mount 残留导致的 cgroup 故障
...
> 符合 v5 §3 一站式模板 + v5 §9 演进型破例 + v5 §10 读者视图规范
