...
- 衔接去:下一篇 [03-Android 文件树全貌 完整挂载点表](03-Android%20文件树全貌%20完整挂载点表.md) 会在本篇"分区 → FS"基础上,讲"挂载后整个文件树的完整图"
...
下一篇 [03-Android 文件树全貌 完整挂载点表](03-Android%20文件树全貌%20完整挂载点表.md)会在本篇"分区 → FS"基础上,**挂载后,给出完整 Android 文件树图**——`/proc` 是什么、`/sys` 是什么、`/dev/ashmem` 是什么、`/storage/emulated/0` 怎么来的——每个挂载点的作用、FS 类型、对应用的影响。
