...
- **Android 17 Mainline APEX 演进**（§1 必覆盖）
- **ART 17 未来演进方向**（§2 必覆盖）
- **ART 17 与 Hook 框架兼容性总结**（§3 必覆盖）
- **整个 ART 系列的 v2 收官总结**（§4）
...
- **Android 17 之前设备**（Android 12-16）—— 也能享受 ART 17 部分强化（§1 已讲）
...
- **AppFunctions 框架**（§3）—— 让 AI Agent 调用 App
- **端侧 LLM 集成**（§4）—— AppFunctions 加载 LLM
...
**Linux 6.18 引入 Rust Binder**（§1 已讲）：
