...
>
> **基线版本**（v4 规范硬要求 · 用户 2026-07-17 决策升级）：AOSP `android-17.0.0_r1`（API 37）+ Linux `android17-6.18`
...
- **不重复内容**：
  ...
  - 不深入类加载（→ 03-类加载与链接）
  - 不深入 JNI（→ 05-JNI）
- **本篇是 v2 示范篇**：ART 顶层 README v2 决策"v2 新增首篇必须按 v4 规范 + AOSP 17 + 6.18 基线写完整"；后续 7 个非 GC 子模块按本篇模板继续
...
| 第 1 轮 · 结构 | **破例记录**（v4 §9 强制）| 实战案例 2 个（规则 1-2 个上限）| **破例理由**：v2 示范篇，要给读者看 v2 规范"实战案例可验证性 4 件套"的样子，1 案例覆盖不足 | 仅本篇 | 否 |
...
3. **"AOT / JIT / interpretation"** —— **3 种执行模式**（v4 §1.3 详解）
...
- **ART 提供 JNI 机制管理 Java/Native 跨界**（v4 §5 JNI 子模块详解）
...
- **JNI Critical 区忘记释放 = 永久死锁**（v4 §5 JNI 子模块详解）
...
| 14 | JNI Critical 泄漏导致死锁 | 5-10s | v4 §5 JNI 子模块 |
...
> **规范**：[PROMPT-技术系列文章写作指南-v4.md](../../../../PROMPT-技术系列文章写作指南-v4.md)
