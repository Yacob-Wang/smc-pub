...
> - **路径校对策略**：本规范 #14 硬要求——附录 B 路径对账表逐条标注【已校对 / 待 v2 校对】
...
- **本规范 #22 硬要求**：附录 B 对账表全量核对完毕（不仅抽查）。后续每篇文章附录 B 都要这么全量
...
## 10. 3 轮校准决策日志（本规范 §7 强制）

### 第 1 轮 · 结构（2026-07-18）

| 决策 | 理由 | 影响范围 |
|------|------|---------|
| 6 章节结构（1 驱动的本质 / 2 数据结构 / 3 入口 / 4 一次拷贝 / 5 BC-BR / 6 实战）| 本规范 #11"本篇定位"硬要求 + 5 大数据结构 1 章、3 大入口 1 章、BC/BR 1 章 | 仅本篇 |
...
| 5 Takeaway 含 1-2 条指向 6.18 硬变化 | 本规范 #12 + 跨篇引用闭环 | 仅本篇 |
...
- **破例记录**（本规范 §9 强制）：
  ...
  | 章节 6 个 | 本规范默认 5-7 | 6.18 vs 6.12 单独一节 + Rust Binder 单独一节需要 6 章 | 仅本篇 | 否 |
