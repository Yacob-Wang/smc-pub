...
> 跨系列引用：见 [Activity · A02 启动流程源码](02_Activity_Start_SourceCode.md) §3.1（Activity 发送广播的链路）—— Activity 在 `ContextImpl.sendBroadcast` 入口的调用模式与 `startActivity` 共享同一 `getAttributionTag()` + `resolveTypeIfNeeded()` 路径。
> 跨系列引用：见 [Service · S02 启动路径](02_Service_StartService_Path.md) §3.1（Service 发送广播的链路）—— Service 在 `ContextImpl.sendBroadcast` 入口的调用模式与 `startService` 共享同一 `getAttributionTag()` + `resolveTypeIfNeeded()` 路径。
> 跨系列引用：见 [ContentProvider · C03 CRUD](C03_ContentProvider_CRUD.md) §2.1（隐式广播 + 跨 App ContentProvider）—— 隐式广播触发 `queryIntentReceivers` 的解析路径与 ContentResolver `acquireProvider` 的跨 App 解析共用 PMS 端 IntentFilter 匹配逻辑。
