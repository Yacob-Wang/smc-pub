...
> 跨系列引用：前台 Service（FGS）类型化与 ANR 检测的关联见 [Service FGS 类型限制](04_Service_FGS_TypeRestricted.md) §3（S04，FGS 类型化与 ANR 检测）；AOSP 16+ AnrHelper 强化的 ANR 检测机制见 [Broadcast ANR 全景](B08_Broadcast_ANR_Landscape.md) §2（B08，ANR 检测 AnrHelper 强化）；ContentProvider publish 的 10s ANR 阈值与 binder 链路见 [ContentProvider Binder ANR](C07_ContentProvider_Binder_ANR.md) §1（C07，ContentProvider ANR 10s）。
