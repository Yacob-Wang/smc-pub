...
> 跨系列引用：见 [Activity · 启动 ANR 案例](07_Activity_Launch_ANR.md)（启动 ANR 案例）
...
> 跨系列引用：见 [Service · bindService 跨进程通信](03_Service_BindService_Path.md)（bindService 泄漏类比）
> 跨系列引用：见 [Broadcast · 动态注册 Receiver 流程](B02_Broadcast_Register.md)（ReceiverDispatcher 泄漏类比）
