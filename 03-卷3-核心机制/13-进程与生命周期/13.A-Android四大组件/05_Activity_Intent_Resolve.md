...
> 跨系列引用：IntentFilter 解析在 PMS 端的具体实现见 [PMS 系列]（待定，PMS 系列未发布）；隐式 Intent + 跨 App ContentProvider 访问的实践场景见 [ContentProvider 跨进程](C04_ContentProvider_CrossProcess.md) §1（C04）。
