...
>
> **v4 规范破例**：本文为**演进对比专题型**（v4 §9 合法破例），单篇规模、图表密度、案例风格均放宽，**详见 §11 决策日志**

---

# 本篇定位

- **本篇系列角色**：**演进对比专题**（v4 §9 合法破例）—— Stability 系列的"演进补充"，独立成篇
- **强依赖**：必先读 [S00-稳定性症状总览](S00-稳定性症状总览.md) + [S01-S07 7 篇](.)
- **承接自**：
  - [S00](S00-稳定性症状总览.md) §3 各症状"关键变化"段（已零散提到 ART 17 / K 6.18 硬变化）
  - [S01-S07](.) 各篇"ART 17 / K 6.18 新变化"子节（已零散覆盖各症状的硬变化）
- **不重复内容**：
  - **不重复** [S00-S07] 7 篇对单症状的深挖（机制层、风险层、案例层都留给那 7 篇）
  - **不重复** [Runtime/ART 系列](../../Runtime/ART/) 142 篇对 ART 17 编译器/类加载/JNI/GC 的全机制深挖
  - **不重复** [Linux_Kernel 系列](../../Linux_Kernel/) 对各子系统的源码深挖
...
| 轮次 | 类别 | 决策 | 理由 | 影响范围 |
|:-----|:-----|:-----|:-----|:---------|
| 1 | 结构 | 单篇 1500+ 行（v4 默认 300 行） | v4 §9 演进对比专题型破例：演进全景需把 6 ART + 8 K + 5 联动 + 5 风险全部讲透 | 仅本篇 |
| 1 | 结构 | 图表 5-7 张（v4 默认 4-6）| v4 §9 演进对比专题型破例：演进全景需多张时序图 + 联动效应图 | 仅本篇 |
...
| 3 | 锐度 | 标注 5 条 takeaway，含 2 条 AOSP 17 + 6.18 硬变化指向 | v4 §4 必备 | §10 |
...
- **上一篇**：[S07-KE](S07-KE.md) 已深挖 KE 机制
...
- **本篇专题类型**：v4 §9 演进对比专题型（破例）

# 写作标准

> 沿用 v4 一站式模板硬性要求 + v4 §9 演进对比专题型破例规则
...
| 已有篇 | 涉及的硬变化 |
|:-------|:-----------|
| [S00](S00-稳定性症状总览.md) §3.1/§3.2/§3.4 | ART 17 分代 GC / AnrHelper 增强 |
| [S01-ANR](S01-ANR.md) | ART 17 MessageQueue 无锁化 / AnrHelper 增强 / K 6.18 Rust Binder |
| [S02-JE](S02-JE.md) | ART 17 static final 不可变 / AppFunctions 引入 |
| [S03-NE](S03-NE.md) | K 6.18 Rust Binder 减少 NE / sheaves 减少 OOM NE |
| [S04-SWT](S04-SWT.md) | ART 17 SystemServer Perfetto 自动 dump |
| [S05-HANG](S05-HANG.md) | ART 17 MessageQueue 无锁化（主线程 HANG 大降）|
| [S06-REBOOT](S06-REBOOT.md) | K 6.18 pstore 增强 / XFS 在线修复 |
| [S07-KE](S07-KE.md) | K 6.18 pstore / sheaves / bcachefs 移除 |
...
| 现有篇 | S08 与之关系 |
|:-------|:------------|
| [S00](S00-稳定性症状总览.md) | **强依赖**——S00 的 7 大症状分类法是 S08 升级收益量化的基础 |
| [S01-ANR](S01-ANR.md) | **横向引用**——S08 提到 ANR 率改善时引 S01 §3 机制 |
| [S02-JE](S02-JE.md) | **横向引用**——S08 提到 AppFunctions JE 时引 S02 §5 异步线程 |
| [S03-NE](S03-NE.md) | **横向引用**——S08 提到 Rust Binder NE 时引 S03 §3 6 种信号 |
| [S04-SWT](S04-SWT.md) | **横向引用**——S08 提到 SystemServer Perfetto 时引 S04 §4 |
| [S05-HANG](S05-HANG.md) | **横向引用**——S08 提到无锁 MQ HANG 改善时引 S05 §3 |
| [S06-REBOOT](S06-REBOOT.md) | **横向引用**——S08 提到 XFS 修复减少 REBOOT 时引 S06 §3 |
| [S07-KE](S07-KE.md) | **横向引用**——S08 提到 K 6.18 pstore 增强时引 S07 §3 |

## 2.2 S08 与其他系列的关系

| 现有系列 | S08 引用 | 关系 |
|:---------|:---------|:-----|
| [Runtime/ART](../../Runtime/ART/) 142 篇 | 强引用（ART 17 机制）| S08 讲"ART 17 影响稳定性"，142 篇讲"ART 17 完整机制" |
| [Linux_Kernel](../../Linux_Kernel/) | 强引用（K 6.18 机制）| S08 讲"K 6.18 影响稳定性"，Linux_Kernel 讲"K 6.18 完整机制" |
| [Runtime/ART/03-GC系统](../../Runtime/ART/03-GC系统/) 99 篇 | 强引用（GC 主题）| S08 讲"分代 GC 影响稳定性"，GC 99 篇讲"分代 GC 完整机制" |
| [AI_Native_X/03_AI_for_Stability](../../AI_Native_X/03_AI_for_Stability/) | 强引用（AI 协同）| S08 提到 AppFunctions / AI Agent OS 时引 AI_for_Stability |
...
- **不重复** [Runtime/ART/01-编译与执行](../../Runtime/ART/02-编译与执行/) 等 142 篇对 ART 机制的全源码深挖
- **不重复** [Linux_Kernel/Memory_Management/MM_v2](../../Linux_Kernel/Memory_Management/MM_v2/) 等对内存管理子系统的源码深挖
...
> **所以呢**：**主线程 4-5s 软卡死的"灰色地带"减少 50%**——HANG 类异常大幅改善（详见 [S05-HANG](S05-HANG.md) §3）。
...
5. **新平台能力 = 新监控需求**——AOSP 17 的 **AppFunctions / AI Agent OS** 是 2026-2027 稳定性新战场，**架构师必须建立专项监控体系**（参见 [AI_Native_X/03_AI_for_Stability](../../AI_Native_X/03_AI_for_Stability/)）。
...
- [S00 总览](S00-稳定性症状总览.md) + [S01 ANR](S01-ANR.md) + [S02 JE](S02-JE.md) + [S03 NE](S03-NE.md) + [S04 SWT](S04-SWT.md) + [S05 HANG](S05-HANG.md) + [S06 REBOOT](S06-REBOOT.md) + [S07 KE](S07-KE.md) + **S08 AOSP17+K6.18 全景（本篇）**
...
> **系列导航**：[← S07-KE](S07-KE.md) | [本系列 README](README-Stability系列.md) | [S00 总览 →](S00-稳定性症状总览.md)
>
