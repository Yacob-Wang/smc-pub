...
> - 分区系列 → `../../../Linux_Kernel/Partition/`(`Zygote` 自身是 `/system/bin/app_process`,其加载与 `/system` partition 启动链路关联)
> - ART 系列 → `../../Runtime/`(preload 阶段预热的 `preloaded-classes` 是 ART 类加载的"母本",本篇 §4 末尾会引到)
...
| fork() 之后子进程的 ART 初始化 | `../../Runtime/`(如该系列存在) | ART 运行时是 Zygote preload 的"产物";05 篇深入 ART |
| `do_fork` / `task_struct` / cgroup | `../../../Linux_Kernel/Partition/`(如该系列存在) | Kernel 视角的进程实现;06 篇展开 |
