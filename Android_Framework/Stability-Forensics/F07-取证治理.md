...
- **本篇系列角色**：**取证治理（横向）**
- **强依赖**：必先读 [F00-取证体系总览](F00-取证体系总览.md) + F01-F06
...
- **不重复内容**：
  ...
  - **不重复** [Tools/Android_Tools](../../Tools/Android_Tools/) 对抓取工具深挖
...
| 篇号 | 标题 | 系列角色 | 关键洞察 |
|:-----|:-----|:---------|:---------|
| [F00](F00-取证体系总览.md) | 取证体系总览 | 全局观 | 症状 × 日志类型 2 维矩阵 + 取证 4 步法 |
| [F01](F01-ANR取证.md) | ANR 取证 | 症状 1/7 | anr traces + dropbox(APP_ANR) + Perfetto |
| [F02](F02-SWT取证.md) | SWT 取证 | 症状 2/7 | watchdog traces + SystemServer Perfetto |
| [F03](F03-JE取证.md) | JE 取证 | 症状 3/7 | dropbox(APP_CRASH) + logcat -b crash |
| [F04](F04-NE取证.md) | NE 取证 | 症状 4/7 | tombstone 16 段 + 符号化服务 |
| [F05](F05-KE取证.md) | KE 取证 | 症状 5/7 | dmesg + pstore + last_kmsg + ramoops |
| [F06](F06-HANG与OOM取证.md) | HANG + OOM 取证 | 症状 6/7 | systrace/ftrace/hprof + 主动监控 |
| [F07](F07-取证治理.md) | 取证治理 | 治理 | APM + bugreport + 商业符号化 |
...
| kernel/pstore | `fs/pstore/` | K 6.12 | pstore 持久化 |
| last_kmsg | `/proc/last_kmsg` | K 6.12 | 重启前最后一次 |
...
1. **5 分钟全局**：[F00](F00-取证体系总览.md) 取证 2 维矩阵 + 4 步法
2. **30 分钟核心**：[F00](F00-取证体系总览.md) + [F01](F01-ANR取证.md) ANR 抓取（最高频）
3. **2 小时深入**：[F00](F00-取证体系总览.md) → [F01](F01-ANR取证.md) → [F04](F04-NE取证.md) NE + 符号化
...
|:-----|:---------------------|:---------------------|
| ANR | [Stability S01](../Stability/S01-ANR.md) §3 机制 | [F01](F01-ANR取证.md) §3 抓取 |
| SWT | [Stability S04](../Stability/S04-SWT.md) §3 机制 | [F02](F02-SWT取证.md) §3 抓取 |
| JE | [Stability S02](../Stability/S02-JE.md) §3 机制 | [F03](F03-JE取证.md) §3 抓取 |
| NE | [Stability S03](../Stability/S03-NE.md) §3 机制 | [F04](F04-NE取证.md) §3 16 段 |
| KE | [Stability S07](../Stability/S07-KE.md) §3 机制 | [F05](F05-KE取证.md) §3 pstore |
| HANG | [Stability S05](../Stability/S05-HANG.md) §3 机制 | [F06](F06-HANG与OOM取证.md) §3 主动抓 |
| OOM | [Stability S02](../Stability/S02-JE.md) §3.5 OOM | [F06](F06-HANG与OOM取证.md) §4-§6 hprof + smaps |
...
>
> **如果只读 1 篇**：推荐 [F00 取证体系总览](F00-取证体系总览.md)（症状 × 日志类型 2 维矩阵 + 取证 4 步法）
>
> **如果排查问题**：先看 [F00 §6 排查路径速查](F00-取证体系总览.md#62-排查路径速查) → 直接跳到对应症状取证篇
>
> **如果团队建设**：先看 [F07 取证治理](F07-取证治理.md) §6 建设路线图

> **系列导航**：[← F06-HANG + OOM 取证](F06-HANG与OOM取证.md) | [本系列 README](README-Forensics系列.md) | [Stability 系列](../Stability/)
>
